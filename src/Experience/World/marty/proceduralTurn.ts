 ...
 */
function generateTurnStepSequence(direction: TurnDirection, totalAngleDeg: number): TurnStepConfig[] {
  ...
  for (let i = 0; i < numSteps; i++) {
    steps.push({ direction, supportingLeg: support, angleDegrees: anglePerStep });
...
export function createProceduralTurnController({
  ...
  const state: ProceduralTurnState = {
    active: false, direction: 'right', totalAngle: 0, completedAngle: 0,
    currentStep: null, phase: 'idle', phaseElapsed: 0, phaseDuration: 0,
  ...
  type HingePlan = { boneName: string; swing: THREE.Quaternion; axis: THREE.Vector3; startAngle: number; targetAngle: number; };
  let hingePlans: HingePlan[] = [];
  
  // Straighten phase ankle reset
  let straightenAnkle: { name: string; swing: THREE.Quaternion; axis: THREE.Vector3; start: number; target: number; } | null = null;
  let supportAnkleAxis: THREE.Vector3 | null = null;

  // ----- BONE HELPERS -----
  
  const findBone = (name: string) => getBoneNodes().find(b => b.name === name);
  const getSupportBones = (leg: SupportingLeg) => LEG_BONES[leg];
  const getFreeBones = (leg: SupportingLeg) => LEG_BONES[leg === 'right' ? 'left' : 'right'];
  ...
  const captureSnapshot = (): Map<string, THREE.Euler> => {
    ...
    getBoneNodes().forEach(bone => snapshot.set(bone.name, bone.object.rotation.clone()));
    return snapshot;
  };
  
  /** Interpolate bone rotations between two snapshots (excludes arms) */
  const interpolateSnapshot = (start: Map<string, THREE.Euler>, target: Map<string, THREE.Euler>, progress: number) => {
  ...
  const applyAnkleFootRotation = (
    ...
    ankleName: string, footName: string,
    degrees: number, visualSign: number
  ) => {
  ...
  const calculateWeightShiftTarget = (step: TurnStepConfig) => {
    ...
    applyAnkleFootRotation(target, support.ankle, support.foot, TURN_ANGLES.weightShift.ankle, sign);
    applyAnkleFootRotation(target, free.ankle, free.foot, TURN_ANGLES.weightShift.ankle, sign);
    return target;
  };
  ...
  const calculateFootLiftTarget = (step: TurnStepConfig) => {
    ...
    const sign = step.supportingLeg === 'right' ? 1 : -1;
    const free = getFreeBones(step.supportingLeg);
    const totalAngle = TURN_ANGLES.weightShift.ankle + TURN_ANGLES.footLift.ankleIncrease;
  ...
  const createHingePlan = (boneName: string, targetAngle: number, axis: THREE.Vector3): HingePlan | null => {
    const bone = findBone(boneName);
    if (!bone) return null;
    const { swing, twist } = decomposeSwingTwist(bone.object.quaternion.clone(), axis);
    return { boneName, swing, axis: axis.clone(), startAngle: getTwistAngle(twist, axis), targetAngle };
  };
  
  /** Setup hinge plans for ankle during weight shift/lift/lower phases */
  const setupAnkleHingePlans = (ankleName: string, degrees: number, visualSign: number) => {
    const initial = getInitialRotation(ankleName);
    if (!initial) return;
    const axis = new THREE.Vector3(1, 0, 0);
    const axisMult = getLegAxisMult(ankleName);
    const baseQ = new THREE.Quaternion().setFromEuler(initial);
    const baseTheta = getTwistAngle(decomposeSwingTwist(baseQ, axis).twist, axis);
    const targetTheta = baseTheta + THREE.MathUtils.degToRad(degrees * visualSign * axisMult);
  ...
  const transitionToPhase = (newPhase: TurnPhase) => {
    ...
    switch (newPhase) {
      case 'shift-weight': {
        ...
        const support = getSupportBones(step.supportingLeg);
        const free = getFreeBones(step.supportingLeg);
        setupAnkleHingePlans(support.ankle, TURN_ANGLES.weightShift.ankle, visualSign);
        setupAnkleHingePlans(free.ankle, TURN_ANGLES.weightShift.ankle, visualSign);
        // Body translation
        if (bodyBone) {
          bodyTargetPos.set(
      ...
      case 'rotate-body': {
        ...
        if (ankleBone && initialEuler) {
          ...
          const delta = baseQ.clone().invert().multiply(ankleBone.object.quaternion.clone());
          supportAnkleAxis = axisFromQuatDelta(delta);
        }
        // Setup pivot rotation
        const { excludedBoneNames, pivotBoneName } = getExcludedBonesForLeg(step.supportingLeg);
        const bones = getBoneNodes();
        pivotBone = bones.find(b => b.name === pivotBoneName) || null;
        pivotExcludedBones = bones.filter(b => excludedBoneNames.includes(b.name));
        if (pivotBone && pivotExcludedBones.length > 0) {
          const model = getModel();
          if (model) {
            pivotState = capturePivotRotationState(findModelRoot(pivotBone.object), pivotBone, pivotExcludedBones);
          }
        }
      ...
      case 'final-reset': {
        ...
        getBoneNodes().forEach(bone => {
          const initial = getInitialRotation(bone.name);
          phaseTargetSnapshot.set(bone.name, initial?.clone() || bone.object.rotation.clone());
        });
  ...
  const completeCurrentStep = () => {
    if (state.currentStep) state.completedAngle += state.currentStep.angleDegrees;
    state.stepIndex++;
  ...
  const getNextPhase = (current: TurnPhase): TurnPhase => {
    const order: TurnPhase[] = ['shift-weight', 'lift-foot', 'rotate-body', 'lower-foot', 'straighten', 'complete'];
  ...
  const startTurn = (direction: TurnDirection, angleDeg: number): Promise<void> => {
  ...
  const update = (deltaSeconds: number) => {
    if (!state.active || state.phase === 'idle' || state.phase === 'complete') return;
    ...
    if (state.phase === 'straighten' && straightenAnkle) {
      ...
      if (bone) {
        const t = easeInOutQuad(progress);
        const theta = lerpAngleShortest(straightenAnkle.start, straightenAnkle.target, t);
        const twist = new THREE.Quaternion().setFromAxisAngle(straightenAnkle.axis, theta);
        bone.object.quaternion.copy(straightenAnkle.swing.clone().multiply(twist));
      }
    }
    ...
    if (bodyBone) {
      const t = easeInOutQuad(progress);
      bodyBone.object.position.x = THREE.MathUtils.lerp(bodyStartPos.x, bodyTargetPos.x, t);
      bodyBone.object.position.y = THREE.MathUtils.lerp(bodyStartPos.y, bodyTargetPos.y, t);
      bodyBone.object.position.z = THREE.MathUtils.lerp(bodyStartPos.z, bodyTargetPos.z, t);
    }
    
    // Apply pivot rotation during rotate-body phase
    if (state.phase === 'rotate-body' && pivotState && pivotBone && pivotExcludedBones.length > 0) {
      const model = getModel();
      if (model) {
        const t = easeInOutCubic(progress);
        const angle = THREE.MathUtils.lerp(pivotStartAngle, pivotTargetAngle, t);
        applyPivotRotationY(angle, findModelRoot(pivotBone.object), pivotBone, pivotExcludedBones, pivotState);
      }
    }
